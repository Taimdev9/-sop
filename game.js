(function () {
    ...
    class World3D {
        ...
        switchToCorridor() {
            ...
            const floorGeo = new THREE.PlaneGeometry(6, CONFIG.ACT3_LENGTH);
            const floor = new THREE.Mesh(floorGeo, floorMat);
            floor.rotation.x = -Math.PI / 2;
            floor.position.set(0, 0, -CONFIG.ACT3_LENGTH / 2);
            this.corridorGroup.add(floor);

            const wallGeo = new THREE.PlaneGeometry(CONFIG.ACT3_LENGTH, 3);

            const leftWall = new THREE.Mesh(wallGeo, wallMat);
            leftWall.rotation.y = Math.PI / 2;
            leftWall.position.set(-3, 1.5, -CONFIG.ACT3_LENGTH / 2);
            this.corridorGroup.add(leftWall);

            const rightWall = new THREE.Mesh(wallGeo, wallMat);
            rightWall.rotation.y = -Math.PI / 2;
            rightWall.position.set(3, 1.5, -CONFIG.ACT3_LENGTH / 2);
            this.corridorGroup.add(rightWall);

            for (let z = -10; z > -CONFIG.ACT3_LENGTH + 20; z -= 15) {
    ...
    const PROLOGUE_DIALOGUE = [
        ...
        {
            speaker: 'DOCTOR',
            text: 'نعرف أنك انتظرت هذا اليوم سنوات طويلة. كل شيء جاهز، خذ نفساً عميقاً... ابدأ التخدير: 3... 2... 1...',
            choices: null
        }
    ];
